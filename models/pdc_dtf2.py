...
def plot_all(freqs, P, name, ch_names):
    ...
    for i in range(N):
        for j in range(N):
            ...
            axes[i, j].set_xticklabels([0, 100],fontsize=4)
            axes[i, j].set_yticklabels([0,0.5],fontsize=4)
            if i == 0:
                axes[i, j].set_ylabel(ch_names[i],fontsize=7)
                axes[i, j].set_title(ch_names[j],fontsize=7)
            else:
                axes[i, j].set_ylabel(ch_names[i],fontsize=7)
    plt.suptitle(name)
    # plt.tight_layout()
